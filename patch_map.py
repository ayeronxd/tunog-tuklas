...
new_style = \"\"\"
        ...
        .node-coastal .node-circle { background: #4CAF50; }
        /*  CANDY LAND (was Rural)  */
        .node-rural .node-circle { background: #E91E63; }
        /*  CRYSTAL SKY (was Highland)  */
        .node-highland .node-circle { background: #00BCD4; }
        /*  ROYAL CASTLE (was Town)  */
        .node-town .node-circle { background: #FF9800; }
        ...
        .map-nodes-layer { position: absolute; inset: 0; z-index: 5; }
        .map-path-svg { position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 4; pointer-events: none; }
        ...
        .active-label {
            ...
            animation: labelBounce 1.4s ease-in-out infinite;
        }
        .active-label::before {
            content: \"\";
            position: absolute;
        ...
        /*  FIXED UI  */
        .pill-fixed { position: fixed; top: 105px; left: 20px; z-index: 9999; }
        .progress-pill {
            background: #fff; border: 4px solid #2c2f33; border-radius: 40px; padding: 10px 22px;
            box-shadow: 0 6px 0 #2c2f33; display: flex; align-items: center; gap: 12px; white-space: nowrap;
        }

        /* Scroll arrows */
        .scroll-hint {
            position: fixed; top: 50%; transform: translateY(-50%); z-index: 500;
            background: #fff; border: 4px solid #2c2f33; border-radius: 50%; width: 55px; height: 55px;
            display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 0 #2c2f33;
            cursor: pointer; transition: all 0.2s; color: #2c2f33; font-size: 1.2rem;
        }
        .scroll-hint:hover { background: #fdd835; color: #000; }
        .scroll-hint:active { transform: translateY(-50%) translateY(4px); box-shadow: none; }
        .scroll-hint-left { left: 14px; }
        .scroll-hint-right { right: 14px; }

        .scroll-progress-bar { position: fixed; bottom: 0; left: 0; width: 100%; height: 8px; background: rgba(0,0,0,0.15); z-index: 9998; }
        .scroll-progress-fill { height: 100%; background: linear-gradient(90deg, #4dcfc6, #fdd835); width: 0%; transition: width 0.1s; border-radius: 0 4px 4px 0; }

        .continue-fixed {
            position: fixed; bottom: 28px; left: 50%; transform: translateX(-50%) translateY(120px);
            z-index: 9999; opacity: 0; pointer-events: none; transition: transform 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275), opacity 0.3s ease;
        }
        .continue-fixed.visible { transform: translateX(-50%) translateY(0); opacity: 1; pointer-events: auto; }
        .continue-btn { box-shadow: 0 8px 0 #2c2f33, 0 12px 30px rgba(0, 0, 0, 0.2) !important; transition: filter 0.15s, box-shadow 0.15s !important; }
        .continue-btn:hover { filter: brightness(1.08); }
        .continue-btn:active { transform: translateY(8px) !important; box-shadow: 0 0 0 #2c2f33 !important; }

        .drag-hint {
            position: fixed; bottom: 50px; left: 50%; transform: translateX(-50%); z-index: 9998;
            color: rgba(255, 255, 255, 0.9); font-size: 0.9rem; font-weight: 800; text-shadow: 1px 1px 0 rgba(0,0,0,0.3);
            display: flex; align-items: center; gap: 6px; animation: fadeHint 3s ease-out forwards; pointer-events: none;
        }
        @keyframes fadeHint { 0%, 60% { opacity: 1; } 100% { opacity: 0; } }
\"\"\"

new_scenery = \"\"\"
                <div class=\"scenery-layer\" aria-hidden=\"true\">
                    <!-- Whimsical Sky Elements -->
...
# Replace <style> to </style> strictly
parts = text.split('<style>')
header = parts[0]
footer_parts = parts[1].split('</style>')
footer = footer_parts[1]

text = header + '<style>\\n' + new_style + '\\n    </style>' + footer

# Replace <div class=\"scenery-layer\" aria-hidden=\"true\"> to </div>
parts = text.split('<div class=\"scenery-layer\" aria-hidden=\"true\">')
header = parts[0]
footer = parts[1].split('<!--  SVG PATH  -->')[1]

text = header + new_scenery + '\\n                <!--  SVG PATH  -->' + footer

# Format SVG glowing paths
text = text.replace('<path class=\"path-glow\" fill=\"none\" filter=\"url(#glow)\" d=\"{{ svg_path_d }}\" />', '<path class=\"path-glow\" fill=\"none\" d=\"{{ svg_path_d }}\" />')

with open(r'c:\Users\User\Desktop\tunog-tuklas\core\templates\core\mapa.html', 'w', encoding='utf-8') as f:
    f.write(text)

print('Success rewriting python map script logic!')